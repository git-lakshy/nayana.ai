...
// Domains & History — dark ASCII-rain header + domain cards (mint-filled when
// selected) + score-over-time area chart with vertical "⚑ fixes applied" flag
// markers and value labels on points, plus a recent-runs paper table with
// delta arrows.
// Wiring: GET /api/domains, GET /api/domains/{domain}/history (FREE+ feature).
...
export default function DomainsPage() {
  ...
  const flagPoints = chartData.filter((p) => p.appliedFixes > 0);

  return (
    <AppShell fullBleed>
      {/* Dark ASCII rain header band */}
      <section className="relative overflow-hidden bg-pine">
        <AsciiRain opacity={0.32} />
        <div
          aria-hidden
        ...
        <div className="relative mx-auto max-w-6xl px-6 pt-10 pb-8">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <BadgeChip
              label={domains.length + " tracked"}
              tone="outline"
              className="ml-auto"
            />
          </div>
          <p className="mt-3 max-w-2xl text-sm text-ink-dim">
            Track your AI visibility score over time — flags mark scans where
            content fixes were applied.
          </p>
        </div>
        <div
          aria-hidden
      ...
      <div className="mx-auto max-w-6xl px-6 py-8">
        {loading ? (
          ...
          <GlassCard className="mt-4 py-14 text-center">
                ...
                return (
                  <button
                    ...
                    className={
                      ...
                      (active
                        ? "border-mint bg-mint/95 text-pine shadow-[0_0_0_4px_rgba(169,229,197,0.15)]"
                        : "border-line bg-frame/50 text-ink hover:border-line-strong")
                    }
                  >
                    <p
                      className={
                    ...
                    <div className="mt-2 flex items-end justify-between">
                      <span
                        className={
                          ...
                          (active ? "text-pine" : "text-mint")
                        }
                      >
                        {active && latest
                          ? Math.round(latest.ai_coverage_score)
                          : "··"}
                      </span>
                      {active && trendValues.length > 1 && (
                        <Sparkline data={trendValues} color="#0e1a16" />
                      )}
                    </div>
                  </button>
                );
                  ...
                  {chartData.length < 2 ? (
                    ...
                    <div className="mt-4 h-80 w-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart
                          data={chartData}
                          margin={{ top: 24, right: 16, bottom: 0, left: -18 }}
                        >
                          <defs>
                            <linearGradient id="mintFill" x1="0" y1="0" x2="0" y2="1">
                              <stop offset="0%" stopColor="#a9e5c5" stopOpacity={0.5} />
                          ...
                          />
                          {/* Vertical flag lines where fixes were applied */}
                          {flagPoints.map((p) => (
                            <ReferenceLine
                              key={"flag-" + p.scanId}
                              x={p.date}
                              ...
                              strokeOpacity={0.75}
                              label={{
                                value: `⚑ ${p.appliedFixes} ${
                                  p.appliedFixes === 1 ? "fix" : "fixes"
                                }`,
                                position: "top",
                                fill: "#e3b341",
                                fontSize: 11,
                          ...
                          <Area
                            ...
                            strokeWidth={2}
                            fill="url(#mintFill)"
                            dot={{ r: 3.5, fill: "#a9e5c5", strokeWidth: 0 }}
                            activeDot={{ r: 5 }}
                          >
                            <LabelList
                              dataKey="score"
                      ...
                      <p className="mt-1 text-right text-xs text-ink-dim">
                        <span className="mr-1 align-middle text-amber">⚑</span>
                      ...
                      <tbody>
                        {history
                          ...
                          .map((row, i, arr) => {
                            ...
                            const delta = prev
                              ? Math.round(
                                  row.ai_coverage_score - prev.ai_coverage_score
                                )
                              : null;
                            return (
                              <tr
                                key={row.scan_id + row.recorded_at}
                              ...
                              >
                                ...
                                <td className="py-2.5 pr-4 font-mono">
                                  {(row.applied_fix_count ?? 0) > 0 && (
                                    <span className="mr-1 text-amber">⚑</span>
                                  )}
                                  {row.applied_fix_count ?? 0}
                                </td>
                                <td className="py-2.5">
                                  <Link
